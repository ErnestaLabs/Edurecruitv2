...
export function AudienceJourney() {
  ...
  return (
    <section id="starting-points" aria-labelledby="journey-heading" className="scroll-mt-24 bg-warm-grey py-20 md:py-28">
      <div className="container-wide px-5 md:px-8">
        <div className="grid gap-10 border-b border-navy/15 pb-10 md:grid-cols-[0.8fr_1.2fr] md:items-end md:gap-20 md:pb-14">
          <div>
            <p className="eyebrow">Choose your starting point</p>
            <h2 id="journey-heading" className="display-section mt-5">See what the first useful conversation could cover for you.</h2>
          </div>
          <p className="max-w-xl text-lg leading-8 text-text-muted">
            Select the situation closest to yours. The advice is personal, but the first step should never feel vague.
          </p>
        </div>
        ...
        <div role="tabpanel" className="journey-panel grid overflow-hidden bg-cream lg:grid-cols-[0.92fr_1.08fr]">
          <div className="relative min-h-[360px] bg-navy md:min-h-[480px] lg:min-h-[620px]">
          ...
          <div className="flex flex-col justify-center p-7 md:p-10 lg:p-14">
            ...
            <h3 className="mt-5 max-w-[14ch] font-heading text-4xl leading-[0.98] text-navy md:text-5xl lg:text-6xl">{active.heading}</h3>
            <p className="mt-6 max-w-xl text-lg leading-8 text-text-muted">{active.description}</p>

            <div className="mt-9 border-t border-navy/15">
              <p className="py-4 text-xs font-extrabold tracking-[0.14em] text-gold uppercase">What we can help you decide</p>
              {active.decisions.map((decision) => (
                <div key={decision} className="flex gap-3 border-t border-navy/12 py-4 text-navy">
            ...
            <a href="#contact" className="button-primary mt-9 w-fit">
