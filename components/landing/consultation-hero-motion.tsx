  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden", background: "transparent", color: "#fcfbf7" }}>
      <div style={{ position: "relative", zIndex: 1, height: "100%", padding: "clamp(1.25rem, 4vw, 2.5rem)" }}>
        <span style={{ position: "absolute", top: "12%", left: "clamp(1.25rem, 4vw, 2.5rem)", borderLeft: "2px solid #c9a84c", paddingLeft: "0.65rem", fontFamily: "Plus Jakarta Sans, sans-serif", fontSize: "0.72rem", fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase" }}>15-minute university plan</span>
        <div style={{ position: "absolute", top: "42%", left: "clamp(1.25rem, 4vw, 2.5rem)", right: "clamp(1.25rem, 4vw, 2.5rem)" }}>
...
export function ConsultationHeroMotion({ imageSrc }: HeroMotionProps) {
  ...
  return <div className="hero-motion-player" aria-hidden="true" style={{ backgroundImage: `linear-gradient(180deg, rgba(16,38,59,0.06), rgba(16,38,59,0.86)), url(${imageSrc})`, backgroundSize: "cover", backgroundPosition: "center" }}><Player component={ConsultationMotion} inputProps={{}} durationInFrames={70} compositionWidth={720} compositionHeight={920} fps={30} autoPlay loop={false} controls={false} style={{ width: "100%", height: "100%", backgroundColor: "transparent" }} /></div>
}
