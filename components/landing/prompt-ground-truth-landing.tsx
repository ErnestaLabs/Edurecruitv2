...
export function PromptGroundTruthLanding() {
  ...
  return <div className="gt-site" data-ready={ready}>
    ...
    <main className="gt-main" id="main-content">
      ...
      <section className="gt-proof gt-section" aria-labelledby="gt-proof-title"><div className="gt-proof-header gt-reveal"><p className="gt-section-eyebrow">Student stories</p><h2 id="gt-proof-title">Support should make the next step feel less lonely.</h2></div><div className="gt-proof-grid">{stories.map((story, index) => <article className="gt-proof-card gt-reveal" style={{ transitionDelay: `${index * 90}ms` }} key={story.name}><div className="gt-proof-person"><Image src={story.src} width={64} height={64} alt={`${story.name}, ${story.designation}`} className="object-cover" /><div><strong>{story.name}</strong><span>{story.designation}</span></div></div><blockquote>“{story.quote}”</blockquote></article>)}</div></section>
